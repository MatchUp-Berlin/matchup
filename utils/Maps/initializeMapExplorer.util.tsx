...
import { cityLatLong } from '../types/Address.Type';
...
export async function initializeMapExplorer(matchUps: MatchUp[], city: TCity) {
  ...
  const markers = matchUps.map((matchUp) => ({
    coordinates: matchUp.address.geometry?.point as [number, number],
    title: `<a style="text-transform: capitalize; font-family: Inter;" href='MatchUps/${matchUp.id}'>${matchUp.title}</a>`,
    address: matchUp?.address?.label,
  }));

  const map = await createMap({
    container: 'map',
    center: center?.geometry?.point as [number, number],
    zoom: 12.5,
  });
