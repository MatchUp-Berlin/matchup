...
export async function createNewOrganized(
  organizedData: Organized
): Promise<Organized | string> {
  try {
    const queryUser = await getUserById(organizedData.userId);
    // if (
    //   queryUser.organized.items.filter(
    //     (el) => el.matchUpId === organizedData.matchUpId
    //   ).length > 0
    // ) {
    ...
    const newOrganizedData = newOrganized.data.createOrganized;
    return newOrganizedData;
  } catch (error) {
    throw error;
  }
}
