...
import { getNumAttendedMatchUps } from '../../utils/Query/getNumAttendedMatchUps.util';
...
const ProfileDetailPage: NextPage = () => {
  ...
  const {
    isLoading: isAttendedCountLoading,
    isSuccess: isAttendedCountSuccess,
    isError: isAttendedCountError,
    data: attendedCountData,
  } = useQuery(['attendedMatchUpsCount', id], () =>
    getNumAttendedMatchUps(id as string)
  );

  // getting organized matchups count
  const {
    isLoading: isOrganizedCount,
    isSuccess: isOrganizedSuccess,
    isError: isOrganizedError,
    data: organizedCountData,
  } = useQuery(['organizedMatchUpsCount', id], () =>
  ...
  return (
        ...
        data && (
          ...
          >
            ...
            <div className={styles.profileMainInfo}>
              <h2 style={{ color: colors.text['100'] }}>
                Hi, my name is {data.givenName}
              </h2>
              <div
              ...
              >
                <div className={styles.singleStat}>
                  ...
                  <p style={{ color: colors.text['60'] }}>
                    Joined in {moment(data.createdAt).format('YYYY')}
                  </p>
                </div>
                <div className={styles.singleStat}>
                  <svg
                  ...
                  <p style={{ color: colors.text['60'] }}>
                    Organized {attendedCountData} MatchUps
                  </p>
                </div>
                <div className={styles.singleStat}>
                  <svg
            ...
            <div className='aboutInfos'>
              ...
              <p style={{ color: colors.text['60'] }}>{data.about}</p>
            </div>
            <Divider />
                  ...
                  data.signups.items.map((signup) => {
                    ...
                    return (
                      <MatchUpCard
                        ...
                        title={signup.matchUp.title}
                        location={signup.matchUp.location}
                        coordinates={signup.matchUp.coordinates}
                        signups={signup.matchUp.signups}
                        organizerId={signup.matchUp.organizerId}
                        sportCategory={signup.matchUp.sportCategory}
                        skillLevel={signup.matchUp.skillLevel}
                        totalCost={signup.matchUp.totalCost}
                        reservedCourt={signup.matchUp.reservedCourt}
                        attendanceMin={signup.matchUp.attendanceMin}
                        attendanceMax={signup.matchUp.attendanceMax}
                        cancelled={signup.matchUp.cancelled}
                        description={signup.matchUp.description}
                        image={signup.matchUp.image}
                        date={signup.matchUp.date}
                        currency={signup.matchUp.currency}
                        updates={signup.matchUp.updates}
                        watchList={signup.matchUp.watchList}
    ...
    // { isLoading ? ( <LoadingSpinner></LoadingSpinner>) : isError ? (<>Oopsie, something went wront</>) :
    // ( isSuccess && data && <section className={styles.contentWrapper}>
    //   <Avatar size={'large'} image={'../../public/basketball.jpg'} />
    //   {<h1>{data?.givenName}</h1>}
    // <Navigation />
    // </section>})
  );
};
