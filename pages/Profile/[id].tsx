...
import GhostMatchUpsCard from '../../components/cards/GhostMatchUpCard';
...
const ProfileDetailPage: NextPage = () => {
  ...
  const { data: organizedCountData, isRefetching: organizedCountIsRefetching } =
    useQuery(
  ...
  const [organizedEventsToken, setOrganizedEventsToken] = useState<
    string | null
  >();
  const organizedQuery = useQuery(
    ['organized', id],
    () =>
      getOrganizerMatchUps(
        id as string,
        organizedEventsToken ? organizedEventsToken : undefined
      ),
    {
      enabled: !!id,
    ...
    return (
      <>
        {currentUser.signups.items.length == 0 ? (
          <Empty
            height='20vh'
            text="You haven't participated in a MatchUp yet."
          />
        ) : (
          <section
          ...
          >
            ...
            <div className={styles.profileMainInfo}>
              ...
              >
                ...
                <div className={styles.detail}>
                  ...
                  <p style={{ color: colors.text['60'] }}>
                    Participated in {currentUser?.signups.items.length || 0}{' '}
                    MatchUps
                  </p>
                </div>
              </div>
            ...
            <div className={styles.matchupsWrapper}>
              ...
              {showNextParticipating <= currentUser?.signups.items.length && (
                <Button
                  variant='secondary'
                  callback={showMoreParticipating}
                  text={'Show more'}
                ></Button>
              )}
            </div>
                      ...
                      {organizedQuery.data?.items.map((organized) => (
                        <>
                          <MatchUpCard
                            ...
                            attendanceMax={
                              organized.matchUp.attendanceMax as number
                            }
                            participating={
                              organized.matchUp.signups?.items?.length || 0
                            }
                            location={organized.matchUp.location as string}
                            sportCategory={
                              organized.matchUp
                                .sportCategory as TSportCategories
                            }
                            skillLevel={
                              organized.matchUp.skillLevel as TSkillLevels
                            }
                            image={organized.matchUp.image as string}
                            totalCost={organized.matchUp.totalCost as number}
                            reservedCourt={
                              organized.matchUp.reservedCourt as boolean
                            }
                          ></MatchUpCard>
          ...
          <div className={styles.profileMainInfo}>
            ...
            <div className={styles.details} style={{ color: colors.text[80] }}>
              ...
              <div className={styles.detail}>
                ...
                <p style={{ color: colors.text['60'] }}>
                  Participated in {userQuery.data?.signups.items.length || 0}{' '}
                  MatchUps
                </p>
              </div>
            </div>
                  ...
                  {organizedQuery.data?.items.map((organized) => (
                    <>
                      <MatchUpCard
                        ...
                        location={organized.matchUp.location as string}
                        sportCategory={
                          organized.matchUp.sportCategory as TSportCategories
                        }
                        skillLevel={
                          organized.matchUp.skillLevel as TSkillLevels
                        }
