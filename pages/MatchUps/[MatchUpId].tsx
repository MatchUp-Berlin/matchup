...
const MatchUpDetail: NextPage = () => {
  ...
  useEffect(() => {
    if (data && user) {
      setIsSignedUp(
        data.signups.items.some(
          (participant) => participant.userId === user.username
        )
      );
  ...
  useEffect(() => {
    if (data && user) {
      ...
      setIsWithin24h(
        Date.parse(data?.date) - ms < Date.parse(new Date().toISOString())
      );
    }
  }, [data, user]);
        ...
        {showParticipantsModal && data && (
          <>
            ...
            <ParticipantsModal
              ...
              participants={
                data.signups.items.map((signup) => signup.user) as User[]
              }
            />
          </>
        )}
