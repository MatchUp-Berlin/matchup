...
const MatchUpDetail: NextPage = () => {
  ...
  const userSignedUp = () => data?.signups.items.some((signup) => signup.userId === user.username) || false;

  useEffect(() => {
    if (data && user) {
      if (userSignedUp()) setIsSignedUp(true);
    }
  }, [data, user]);

  const eventHasStarted = () => {
    return data && Date.parse(data?.date) < Date.parse(new Date().toISOString());
  };

  const isWithin24Hours = () => {
    const twentyFourHoursInMilSec = 24 * 60 * 60 * 1000;
    return (
      (data && Date.parse(data?.date) - twentyFourHoursInMilSec < Date.parse(new Date().toISOString())) ||
      false
    );
  };
        ...
        {showParticipantsModal && data && (
          <>
            ...
            <ParticipantsModal
              ...
              participants={data.signups.items as Partial<User>[]}
            />
          </>
        )}
        ...
        {showUpdatesModal && data && (
          <>
            ...
            <UpdatesModal
              ...
              updates={data.updates.items as Update[]}
