...
const MatchUpDetail: NextPage = () => {
  ...
  useEffect(() => {
    if (matchUp && currentUserId) {
      setHasFinished(
        Date.parse(matchUp?.date) < Date.parse(new Date().toISOString())
      );
      setIsWithin24h(
        Date.parse(matchUp?.date) - ms < Date.parse(new Date().toISOString())
      );
    }
  }, [matchUp, currentUserId]);
        ...
        {showSignUpModal && matchUp && (
          <>
            <div
              ...
              style={{ backgroundColor: colors.overlay[100] }}
            ></div>
            <ConfirmJoinModal
              matchUp={matchUp}
            ...
            <div className={styles.contentWrapper}>
              ...
              <div className={styles.bigPills}>
                <BigPill
                  filled
                  title='Skill'
                  text={matchUp.skillLevel}
                ></BigPill>
                <BigPill
                  filled={
                    matchUp.attendanceMax - matchUp.signups.items.length == 0
                  }
          ...
          leftSide={
            <div className={styles.footerInfo}>
              <p className='fat' style={{ color: colors.text[100] }}>
                {matchUp?.signups.items.length} / {matchUp?.attendanceMax}{' '}
                players joined
              </p>
              <p className='small' style={{ color: colors.text[100] }}>
                {matchUp?.totalCost > 0 ? matchUp?.totalCost + '.00€' : 'Free'}{' '}
                + 5€ deposit{' '}
                {
                  <span>
                    <svg
