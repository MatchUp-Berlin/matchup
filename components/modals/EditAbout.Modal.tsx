...
import styles from './styles/EditAbout.Modal.module.scss';
import SmallButton from '../misc/SmallButton';
import { Button } from '../misc';

export interface IEditAboutModalProps {
  close: () => void;
  about: string;
  newAbout: string;
  setNewAbout: (e: any) => void;
  save: () => void;
}

const EditAboutModal: React.FunctionComponent<IEditAboutModalProps> = (props) => {
    ...
    <>
      ...
      >
        ...
        <textarea
          ...
          value={props.newAbout.length == 0 ? props.about : props.newAbout}
          onChange={props.setNewAbout}
          style={{ backgroundColor: 'transparent', color: colors.text[100], fontSize: '1em' }}
        >
          {props.about}
        </textarea>
        <div className={styles.saveButton}>

        <Button variant="primary" text="Save" callback={props.save}></Button>
        </div>
      </div>
    </>
  );
...
return (
  <>
    ...
    {display && (
      <EditAboutModal participants={participants} close={() => setDisplay(false)}></EditAboutModal>
    )}
  </>
);
